...
import {
  BacktrackingEvmEvent, BytecodeLog, CompleteEvmDataV1, Contract, DecodedEvmError, ERC20Token, EventBody, EVMEventData, VerifiedContract,
} from '../types';
import {
  insertOne, query, queryv2,
...
const parseEvmLogData = async (method: string, genericData: GenericEventData): Promise<undefined|CompleteEvmDataV1> => {
  ...
  if (method === 'Log') {
    ...
    const evmData: CompleteEvmDataV1 = {
...
interface InsertEvmLog extends CompleteEvmDataV1, EvmBase {
  method: 'Log',
  status: 'Success';
  type: 'Verified' | 'Unverified';
}

interface InsertEvmEventFailure extends CompleteEvmDataV1, EvmBase {
